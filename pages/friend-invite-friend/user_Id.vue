...
const employeeIdFromQuery = computed(() =>
  String(route.query.employee_id || route.query.employeeId || "").trim(),
);
...
async function loadShareQrByEmployeeId(employeeId: string) {
  await runShareLoader(async () => {
...
onMounted(() => {
  if (employeeIdFromQuery.value) {
    void loadShareQrByEmployeeId(employeeIdFromQuery.value);
  } else if (employeeShareIdFromQuery.value) {
