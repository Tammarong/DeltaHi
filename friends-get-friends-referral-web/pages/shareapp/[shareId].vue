...
const actionErrorText = computed(() =>
  actionError.value ? t(actionError.value.key, actionError.value.params ?? {}) : ''
)
    ...
    <section class="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-md flex-col justify-center">
      <div class="delta-glow-card">
        ...
        <form v-else class="mt-6 space-y-4" @submit.prevent="submitEmployeeId">
          ...
          <label class="block">
            <span class="text-sm font-medium text-slate-800">{{ t('shareApp.employeeId.label') }}</span>
            <input
              v-model="employeeId"
              type="text"
              inputmode="text"
          ...
          <p v-if="formErrorText" class="rounded-md bg-red-50 p-3 text-sm text-red-700">
            {{ formErrorText }}
          </p>
    ...
    >
      ...
      >
        <div class="flex items-start justify-between gap-4">
          <div>
            <p class="text-sm font-medium text-emerald-700">{{ t('shareApp.dialog.ready') }}</p>
        ...
        <div v-if="downloadStep === 'download'">
          ...
          >
            {{ t('shareApp.employeeLookup.verified', { name: downloadReceiverEmployee.displayName }) }}
          </div>
          <div
          ...
          <div
            ...
            class="mt-5 rounded-md bg-amber-50 p-3 text-sm text-amber-800"
          >
            {{ t('shareApp.employeeLookup.notFoundDownload', { employeeId: downloadReceiverEmpId }) }}
          </div>
          ...
          <button
            ...
            @click="downloadApp"
          >
            {{ isRecordingClick ? t('shareApp.actions.checkingDevice') : t('shareApp.actions.downloadApp') }}
          </button>

          <div class="mt-5 border-t border-slate-200 pt-5 text-center">
            <p class="text-sm text-slate-600">{{ t('shareApp.dialog.cannotDownload') }}</p>
            <button
              type="button"
        ...
        <div v-else-if="downloadStep === 'choose-os'">
          <p class="mt-5 text-sm font-medium text-brand-700">{{ t('shareApp.tutorial.label') }}</p>
          <h2 class="mt-2 text-2xl font-semibold text-slate-950">{{ t('shareApp.tutorial.choosePhone') }}</h2>
        ...
        <div v-else>
          <p class="mt-5 text-sm font-medium text-brand-700">
            {{ downloadStep === 'ios' ? t('shareApp.tutorial.iosLabel') : t('shareApp.tutorial.androidLabel') }}
          </p>
          <h2 class="mt-2 text-2xl font-semibold text-slate-950">
            {{ downloadStep === 'ios' ? t('shareApp.tutorial.installIos') : t('shareApp.tutorial.installAndroid') }}
          </h2>

          <div v-if="downloadStep === 'ios'" class="mt-5 space-y-4 overscroll-contain">
            <div
            ...
            >
              <img
                :src="imageUrl"
                :alt="t('shareApp.tutorial.iosImageAlt', { step: imageIndex + 1 })"
          ...
          <div v-else class="mt-5 space-y-4 overscroll-contain">
            ...
            >
              <img
                :src="imageUrl"
                :alt="t('shareApp.tutorial.androidImageAlt', { step: imageIndex + 1 })"
                class="mx-auto w-full object-contain"
              >
            </div>
          </div>
        ...
        <p v-if="actionErrorText" class="mt-4 rounded-md bg-red-50 p-3 text-sm text-red-700">
          {{ actionErrorText }}
        </p>
      </div>
    </div>
