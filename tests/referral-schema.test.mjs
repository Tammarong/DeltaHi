...
test('employee shares persist and expose the referrer employee name', () => {
  ...
  const qrPage = readProjectFile('pages/friend-invite-friend/user_Id.vue')
  ...
  assert.match(qrPage, /pointBalance:\s*number \| null/)
  assert.match(qrPage, /response\.data\.point_balance \?\? response\.data\.pointBalance \?\? null/)
})
...
test('first page creates a QR from the check-user API response', () => {
  const indexPage = readProjectFile('pages/friend-invite-friend/user_Id.vue')
  ...
  assert.match(indexPage, /const employeeIdFromQuery = computed/)
  assert.match(indexPage, /route\.query\.employee_id/)
  assert.match(indexPage, /type CheckUserResponse =/)
  assert.match(indexPage, /point_balance\?: number \| string \| null/)
  ...
  assert.match(indexPage, /async function loadShareQrByEmployeeId\(employeeId: string\)/)
  assert.match(indexPage, /\$fetch<CheckUserResponse>\(\s*["']\/api\/friend-invite-friend\/check-user["']/)
  assert.match(indexPage, /employee_id:\s*employeeId/)
  assert.match(indexPage, /\$fetch<GetEmployeeShareResponse>\(\s*["']\/api\/employee-shares["']/)
  assert.match(indexPage, /userId:\s*response\.data\.id/)
  assert.match(indexPage, /employeeId:\s*response\.data\.employee_id/)
  assert.match(indexPage, /pointBalance:\s*[\s\S]*response\.data\.point_balance/)
  ...
  assert.match(indexPage, /void loadShareQrByEmployeeId\(employeeIdFromQuery\.value\)/)
